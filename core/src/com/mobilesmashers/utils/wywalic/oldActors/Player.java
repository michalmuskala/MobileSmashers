...
public class Player extends com.mobilesmashers.utils.wywalic.oldActors.GameActor {

	private List<com.mobilesmashers.utils.wywalic.oldActors.Hook> inventory;

	public Player(Vector2 position, Vector2 size, Texture texture) {
		super(position, size, new Vector2(0, 0), texture);
		inventory = new ArrayList<com.mobilesmashers.utils.wywalic.oldActors.Hook>();
	}
	...
	public void addToInventory(com.mobilesmashers.utils.wywalic.oldActors.Hook hook) {
		inventory.add(hook);
	}

	public void shoot(float dirX, float dirY, float force) {
		if (inventory.size() > 0) {
			com.mobilesmashers.utils.wywalic.oldActors.Hook hook = inventory.get(0);
			inventory.remove(0);
